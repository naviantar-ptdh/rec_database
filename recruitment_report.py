...
if mpp.empty:
    ...
    if "loc" in mpp.columns:
        ...
        loc_sel = f1.selectbox("Location (MPP)", loc_opt, key="mpp_loc")
    ...
    if "level" in mpp.columns:
        ...
        lvl_sel = f2.selectbox("Level (MPP)", lvl_opt, key="mpp_level")

        if lvl_sel != "All":
            mpp_filtered = mpp_filtered[mpp_filtered["level"] == lvl_sel]

    # STATUS (BARU - KHUSUS MPP)
    if "status" in mpp.columns:
        status_opt = ["All"] + sorted(mpp["status"].dropna().unique())
        status_sel = f3.selectbox("Status (MPP)", status_opt, key="mpp_status")

        if status_sel != "All":
            mpp_filtered = mpp_filtered[
                mpp_filtered["status"].str.upper() == status_sel.upper()
            ]
